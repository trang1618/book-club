...
co-occurrences and correlations, tf-idf, and topic modeling to explore the connections between the datasets.
...
nasa_keyword %>% 
  count(keyword) %>% 
  slice_max(n, n = 30) %>% 
  mutate(keyword_type = case_when(
    grepl('ocean', keyword) ~ 'ocean',
    grepl('center|lab|project|complete|active', keyword) ~ 'laboratory',
    TRUE ~ 'earth+'
  )) %>% 
  ggplot(aes(n, forcats::fct_reorder(keyword, n), fill = keyword_type)) +
  geom_col() +
  coord_cartesian(expand = FALSE) +
  labs(x = 'Word count', y = NULL, fill = NULL,
...
set.seed(1234)
desc_word_pairs %>%
  filter(n >= 6000, item1 != 'data', item2 != 'data') %>%
  graph_from_data_frame() %>%
  ggraph(layout = "fr") +
...
desc_tf_idf %>% 
  filter(!near(tf, 1)) %>%
  filter(keyword %in% tolower(c("SOLAR ACTIVITY", "CLOUDS", 
